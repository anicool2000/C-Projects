...
struct deccan_airline
{
     ...
     int seat_num;
     char email[15];
     struct deccan_airline *following;
}
*begin, *stream;
struct deccan_airline *dummy;
...
void main()
{
      ...
      printf("\n\n\t\t ********************************************************************");
		printf("\n\t\t                   welcome to Aniruddh's airline system                   ");
		printf("\n\t\t   *******************************************************************");
...
void reserve(int x)
{
      ...
      if(begin == NULL)
      {
          begin = stream = (struct deccan_airline*)malloc(sizeof(struct deccan_airline));
          details();
      ...
      else
      {
          ...
          stream->following = (struct deccan_airline *)malloc(sizeof(struct deccan_airline));
          stream = stream->following;
          details();
          stream->following = NULL;
          printf("\n\t Seat booking successful!");
...
void savefile()
{
    FILE *fpointer = fopen("deccan records","w");
    if (!fpointer)
    {
    ...
    printf("\n\n\t Details have been saved to a file (deccan records)");
    fclose(fpointer);
}
